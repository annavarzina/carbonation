...
settings = {'precipitation': 'interface', # 'interface'/'all'/'mineral' nodes
            ...
            'diffusivity':{'border': D, ##diffusivity at border
                           ...
                           'CC': ('const', 1e-11), # fixed diffusivity in portlandite node 'archie'/'const'/'inverse'
                           }, 
...
plist =  [(1,n) for n in np.arange(0, 15)] #points list
...
comp =  [ 'poros (1, 1)','poros (1, 2)', 'poros (1, 3)',  'poros (1, 4)',
         'poros (1, 5)', 'poros (1, 6)',  'poros (1, 7)',  'poros (1, 8)', 
         'poros (1, 9)',  'poros (1, 10)', ]
thres = 1e-5
for p in comp:
    for i in range(1,len(results[p])-1):
        if np.abs(results[p][i] - results[p][i-1]) < thres:

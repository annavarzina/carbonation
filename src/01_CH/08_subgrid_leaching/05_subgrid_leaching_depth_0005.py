...
solver_params = fn.set_solver_params(tfact = 1./6., smart_thres = 1e-8)#, cphi_fact = 1./3.*2)# optional values, for time step (if tfact => tfactbased tau)
...
plt.figure()
plt.plot(rt_time[2:-1], np.abs(dport[2:-1]))
plt.xlabel("Time")

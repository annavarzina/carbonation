...
class CarbonationRT(PhrqcReactiveTransport):
    ...
    def advance(self):
        #print(self.iters)
        #print(self.solid.portlandite.c[1,:])
    ...
    def update_diffusivity_fixed(self):
        ...
        De = Dref * self.solid.poros * self.solid.app_tort 
        if(D_CC is None and D_CH is None): 
            
            De = D_border*is_border + Dref*is_liquid + De*is_port + De*is_calc
        elif(D_CC is None and D_CH is not None):
            De = D_CH*is_port+ D_border*is_border + Dref*is_liquid + De*is_calc
        elif(D_CC is not None and D_CH is None):            
            De = D_CC*is_calc+ D_border*is_border + Dref*is_liquid + De*is_port 
        else:

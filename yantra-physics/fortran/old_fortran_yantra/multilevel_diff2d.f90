...
!of multilevel 2D diffusion equation using TRT scheme.
!The relaxation parameter tau can be varying in domain
!
!=======================================================================================
Subroutine compute_macro_var (f, c, flux, poros, nodetype, tau_a, ly, &
& lx)
         ...
         Do i = 1, ly
            ...
            If (nodetype(i, j) <= 0) Then
              ...
              & (i, j, 4) + f (i, j, 5)
               ...
               flux (1, i, j) = (1-(1./(2.*tau_a(i, j)))) * (f(i, j, &
              & 2)-f(i, j, 4))
               flux (2, i, j) = (1-(1./(2.*tau_a(i, j)))) * (f(i, j, &
              & 3)-f(i, j, 5))
            End If
         End Do
      End Do
...
!
Subroutine compute_edf (f, c, cphi, poros, nodetype, ly, lx)
      Implicit None
    ...
    !$OMP DO
      Do j = 1, lx
         Do i = 1, ly
            If (nodetype(i, j) <= 0) Then
              !f2
               f (i, j, 2) = w * c (i, j) * cphi
              !f3
               f (i, j, 3) = w * c (i, j) * cphi
              !f4
               f (i, j, 4) = w * c (i, j) * cphi
              !f5
               f (i, j, 5) = w * c (i, j) * cphi
              !f1
         ...
         Do j = 1, lx
            If (nodetype(1, j) <= 0.) Then
               If (interp == 1) Then
                 ...
                 & j, 4) + f (2, j, 5)
                  flux0 = (1.-(1./(2.*tau_a(2, j)))) * (f(2, j, 3)-f(2, &
                 & j, 5))
            ...
            If (nodetype(ly, j) <= 0.) Then
               If (interp == 1) Then
                 ...
                 & + f (ly-1, j, 4) + f (ly-1, j, 5)
                  flux0 = (1.-(1./(2.*tau_a(ly-1, j)))) * (f(ly-1, j, &
                 & 3)-f(ly-1, j, 5))
         ...
         Do i = 1, ly
            If (nodetype(i, 1) <= 0.) Then
               If (interp == 1) Then
                 ...
                 & 2, 4) + f (i, 2, 5)
                  flux0 = (1.-(1./(2.*tau_a(i, 2)))) * (f(i, 2, 2)-f(i, &
                 & 2, 4))
            ...
            If (nodetype(i, lx) <= 0.) Then
               If (interp == 1) Then
                 ...
                 & + f (i, lx-1, 4) + f (i, lx-1, 5)
                  flux0 = (1.-(1./(2.*tau_a(i, lx-1)))) * (f(i, lx-1, &
                 & 2)-f(i, lx-1, 4))
